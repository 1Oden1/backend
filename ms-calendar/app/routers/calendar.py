...
@internal_router.get("/filieres/{filiere_id}/enseignants")
def internal_enseignants_by_filiere(filiere_id: int, db: Session = Depends(get_db)):
    """
    Retourne les enseignants qui ont au moins une séance dans la filière donnée.
    Utilisé par ms-notes pour la route /etudiant/enseignants-filiere.
    """
    from sqlalchemy import select as sa_select, distinct
    rows = db.execute(
        sa_select(distinct(Enseignant.id), Enseignant.user_id, Enseignant.nom,
...
@internal_router.get("/enseignants")
def internal_list_all_enseignants(db: Session = Depends(get_db)):
    """Retourne TOUS les enseignants — fallback sans filtre de filière."""
    rows = db.scalars(select(Enseignant).order_by(Enseignant.nom)).all()
    return {
        "enseignants": [
